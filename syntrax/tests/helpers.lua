...
function mod.assert_compilation_succeeds(source, initial_rail, initial_direction)
   ...
   if err then
      ...
      lu.fail(string.format(
         "Expected compilation to succeed for:\n%s\nBut got error: %s",
         source,
         err.message
      ))
   end
...
function mod.assert_compilation_fails(source, expected_error_code, expected_message_pattern)
   ...
   if expected_message_pattern then
      lu.assertStrContains(err.message, expected_message_pattern)
   end
   return err
end
...
function mod.assert_rail_sequence(rails, expected_kinds)
   lu.assertEquals(#rails, #expected_kinds, 
      string.format("Expected %d rails but got %d", #expected_kinds, #rails))
   for i, expected_kind in ipairs(expected_kinds) do
      lu.assertEquals(rails[i].kind, expected_kind,
...
function mod.assert_rail_direction(rails, rail_index, expected_direction, direction_type)
   ...
   lu.assertEquals(actual, expected_direction,
      string.format("Rail %d %s direction", rail_index, direction_type))
end
...
function mod.run_test_case(test_case)
   if test_case.expected_error then
      ...
      local rails = mod.assert_compilation_succeeds(
         test_case.source,
         test_case.initial_rail,
         test_case.initial_direction
      )
      
      lu.assertEquals(#rails, test_case.expected_count)
      
      if test_case.expected_kinds then
         mod.assert_rail_sequence(rails, test_case.expected_kinds)
      end
